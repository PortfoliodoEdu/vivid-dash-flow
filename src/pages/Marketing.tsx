...
export default function Marketing() {
  ...
  const custoPorLead = totalInvestimento / totalLeads;
  const conversaoMedia = (canalData.reduce((acc, curr) => acc + curr.conversao, 0) / canalData.length).toFixed(1);
  const roiMedio = roiMensal[roiMensal.length - 1].roi;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6 gradient-card border-border shadow-soft">
          ...
          <div className="mt-6 pt-4 border-t border-border">
            <p className="text-xs text-muted-foreground">
              <strong>Taxa de Conversão Global:</strong> {((funnelData[4].valor / funnelData[0].valor) * 100).toFixed(2)}%
            </p>
          </div>
        </Card>
      </div>
